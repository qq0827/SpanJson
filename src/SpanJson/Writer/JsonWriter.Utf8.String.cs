namespace SpanJson
{
    ...
    {
        public void WriteUtf8EscapedString(string value)
        {
            WriteUtf8StringEscapedValue(value.AsSpan(), false);
        }

        public void WriteUtf8EscapedString(in ReadOnlySpan<char> value)
        {
            WriteUtf8StringEscapedValue(value, false);
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void WriteUtf8StringEscapedValue(in ReadOnlySpan<char> value, bool withNameSeparator)
        {
            ...
            pos += TextEncodings.Utf8.GetBytes(value, Utf8Span);
            WriteUtf8DoubleQuote(ref pinnableAddr, ref pos);
        ...
        private void WriteUtf8StringEscapeValue(in ReadOnlySpan<char> value, bool withNameSeparator)
        {
            ...
            while ((uint)index < nValueLength)
            {
                ...
                if (EscapingHelper.Default.NeedsEscaping(val))
                {
                    ...
                    EscapingHelper.EscapeNextChars(ref utf16Source, nValueLength, val, ref pinnableAddr, ref index, ref pos);
                    index++;
