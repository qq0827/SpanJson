...
namespace SpanJson
{
    partial struct Utf8JsonWriter
    {
        ...
        /// </remarks>
        public void WriteNumber(in JsonEncodedText propertyName, decimal value)
            => WriteNumberHelper(propertyName.EncodedUtf8Bytes, value);

        private void WriteNumberHelper(in ReadOnlySpan<byte> utf8PropertyName, decimal value)
        {
        ...
        /// The property name is escaped before writing.
        /// </remarks>
        public void WriteNumber(in ReadOnlySpan<char> propertyName, decimal value)
        {
        ...
        /// The property name is escaped before writing.
        /// </remarks>
        public void WriteNumber(in ReadOnlySpan<byte> utf8PropertyName, decimal value)
        {
        ...
        private void WriteNumberEscape(in ReadOnlySpan<char> propertyName, decimal value)
        {
        ...
        private void WriteNumberEscape(in ReadOnlySpan<byte> utf8PropertyName, decimal value)
        {
        ...
        private void WriteNumberEscapeProperty(in ReadOnlySpan<byte> utf8PropertyName, decimal value, int firstEscapeIndexProp)
        {
        ...
        private void WriteNumberByOptions(in ReadOnlySpan<byte> utf8PropertyName, decimal value)
        {
            ValidateWritingProperty();
            if (_options.Indented)
            {
        ...
        {
            Debug.Assert(escapedPropertyName.Length < (int.MaxValue / JsonSharedConstant.MaxExpansionFactorWhileTranscoding) - JsonSharedConstant.MaximumFormatDecimalLength - 4);

            // All ASCII, 2 quotes for property name, and 1 colon => escapedPropertyName.Length + JsonSharedConstant.MaximumFormatDecimalLength + 3
            // Optionally, 1 list separator, and up to 3x growth when transcoding
            int maxRequired = (escapedPropertyName.Length * JsonSharedConstant.MaxExpansionFactorWhileTranscoding) + JsonSharedConstant.MaximumFormatDecimalLength + 4;
        ...
        private void WriteNumberMinimized(in ReadOnlySpan<byte> escapedPropertyName, decimal value)
        {
            ...
            Debug.Assert(nameLen < int.MaxValue - JsonSharedConstant.MaximumFormatDecimalLength - 4);

            int minRequired = nameLen + JsonSharedConstant.MaximumFormatDecimalLength + 3; // 2 quotes for property name, and 1 colon
        ...
        private void WriteNumberIndented(in ReadOnlySpan<char> escapedPropertyName, decimal value)
        {
            ...
            Debug.Assert(escapedPropertyName.Length < (int.MaxValue / JsonSharedConstant.MaxExpansionFactorWhileTranscoding) - indent - JsonSharedConstant.MaximumFormatDecimalLength - 5 - JsonWriterHelper.NewLineLength);

            // All ASCII, 2 quotes for property name, 1 colon, and 1 space => escapedPropertyName.Length + JsonSharedConstant.MaximumFormatDecimalLength + 4
            // Optionally, 1 list separator, 1-2 bytes for new line, and up to 3x growth when transcoding
            int maxRequired = indent + (escapedPropertyName.Length * JsonSharedConstant.MaxExpansionFactorWhileTranscoding) + JsonSharedConstant.MaximumFormatDecimalLength + 5 + JsonWriterHelper.NewLineLength;
        ...
        private void WriteNumberIndented(in ReadOnlySpan<byte> escapedPropertyName, decimal value)
        {
            ...
            int nameLen = escapedPropertyName.Length;
            Debug.Assert(nameLen < int.MaxValue - indent - JsonSharedConstant.MaximumFormatDecimalLength - 5 - JsonWriterHelper.NewLineLength);

            int minRequired = indent + nameLen + JsonSharedConstant.MaximumFormatDecimalLength + 4; // 2 quotes for property name, 1 colon, and 1 space
