...
namespace SpanJson.Document
{
    public sealed partial class JsonDocument
    {
        ...
        private struct MetadataDb : IDisposable
        {
            ...
            internal MetadataDb(byte[] completeDb, bool isDisposable)
            {
                _data = completeDb;
            ...
            internal void TrimExcess()
            {
                ...
                {
                    byte[] newRent = _isDisposable ? ArrayPool<byte>.Shared.Rent(Length) : new byte[Length];
                    byte[] returnBuf = newRent;

                    if (newRent.Length < _data.Length)
                    {
                        BinaryUtil.CopyMemory(_data, 0, newRent, 0, Length);
                        returnBuf = _data;
                        _data = newRent;
                    }
            ...
            internal void Append(JsonTokenType tokenType, int startLocation, int length)
            {
                ...
                Debug.Assert(
                    (tokenType == JsonTokenType.BeginArray || tokenType == JsonTokenType.BeginObject) ==
            ...
            {
                byte[] toReturn = _data;
                _data = _isDisposable ? ArrayPool<byte>.Shared.Rent(toReturn.Length * 2) : new byte[toReturn.Length * 2];
                BinaryUtil.CopyMemory(toReturn, 0, _data, 0, toReturn.Length);
            ...
            {
                Debug.Assert(lookupType == JsonTokenType.BeginObject || lookupType == JsonTokenType.BeginArray);
            ...
            internal MetadataDb CopySegment(int startIndex, int endIndex)
            {
                ...
                if (start.TokenType == JsonTokenType.BeginObject)
                {
                    Debug.Assert(
                        end.TokenType == JsonTokenType.EndObject,
                        $"StartObject paired with {end.TokenType}");
                }
                else if (start.TokenType == JsonTokenType.BeginArray)
                {
                    Debug.Assert(
                ...
                return new MetadataDb(newDatabase, false);
            }
        }
    }
}
