...
namespace SpanJson
{
    public ref partial struct Utf8JsonReader
    {
        ...
        private bool ReadMultiSegment()
        {
            ...
            if (_tokenType == JsonTokenType.BeginObject)
            {
        ...
        {
            if (first == JsonUtf8Constant.OpenBrace)
            {
            ...
            else if (first == JsonUtf8Constant.OpenBracket)
            {
        ...
        private bool ConsumeValueMultiSegment(byte marker)
        {
            while (true)
            {
                ...
                else if (marker == JsonUtf8Constant.OpenBrace)
                {
                    StartObject();
                }
                else if (marker == JsonUtf8Constant.OpenBracket)
                {
                    StartArray();
                }
                ...
                {
                    return ConsumeLiteralMultiSegment(JsonUtf8Constant.FalseValue, JsonTokenType.False);
                }
                else if (marker == 't')
                {
                    return ConsumeLiteralMultiSegment(JsonUtf8Constant.TrueValue, JsonTokenType.True);
                }
                else if (marker == 'n')
                {
                    return ConsumeLiteralMultiSegment(JsonUtf8Constant.NullValue, JsonTokenType.Null);
                }
                else
                {
        ...
        private bool ConsumeLiteralMultiSegment(in ReadOnlySpan<byte> literal, JsonTokenType tokenType)
        {
        ...
        private int FindMismatch(in ReadOnlySpan<byte> span, in ReadOnlySpan<byte> literal)
        {
            Debug.Assert(span.Length > 0);
        ...
        private JsonException GetInvalidLiteralMultiSegment(in ReadOnlySpan<byte> span)
        {
            ...
            return SysJsonThrowHelper.GetJsonReaderException(ref this, resource, nextByte: default, bytes: span);
        }
        ...
        private bool ConsumePropertyNameMultiSegment()
        {
            ...
            if (first != JsonUtf8Constant.KeyValueSeperator)
            {
                    ...
                    else
                    {
                        ...
                        StartOfLoop:
                            for (; idx < localBuffer.Length; idx++)
                            {
                                ...
                                else if (currentByte == JsonUtf8Constant.BackSlash)
                                {
        ...
        private ConsumeTokenResult ConsumeNextTokenMultiSegment(byte marker)
        {
            ...
            if (marker == JsonUtf8Constant.ListSeparator)
            {
                _consumed++;
                _bytePositionInLine++;
            ...
            else if (marker == JsonUtf8Constant.CloseBrace)
            {
                EndObject();
            }
            else if (marker == JsonUtf8Constant.CloseBracket)
            {
                EndArray();
            }
            else
        ...
        private ConsumeTokenResult ConsumeNextTokenFromLastNonCommentTokenMultiSegment()
        {
            ...
            if (first == JsonUtf8Constant.ListSeparator)
            {
            ...
            else if (first == JsonUtf8Constant.CloseBrace)
            {
                EndObject();
            }
            else if (first == JsonUtf8Constant.CloseBracket)
            {
                EndArray();
            }
        ...
        private ConsumeTokenResult ConsumeNextTokenUntilAfterAllCommentsAreSkippedMultiSegment(byte marker)
        {
            ...
            else if (marker == JsonUtf8Constant.ListSeparator)
            {
                _consumed++;
                _bytePositionInLine++;
        ...
        private bool SkipCommentMultiSegment(out int tailBytesToIgnore)
        {
            ...
            if (marker != JsonUtf8Constant.Slash && marker != JsonUtf8Constant.Asterisk)
            {
                SysJsonThrowHelper.ThrowJsonReaderException(ref this, ExceptionResource.InvalidCharacterAtStartOfComment, marker);
            }

            bool multiLine = marker == JsonUtf8Constant.Asterisk;
        ...
        private bool SkipSingleLineCommentMultiSegment(ReadOnlySpan<byte> localBuffer, out int tailBytesToSkip)
        {
            ...
            while (true)
            {
                if (expectLF)
                {
                    if (localBuffer[0] == JsonUtf8Constant.LineFeed)
                    {
                        tailBytesToSkip++;
                        _consumed++;
                    }
        ...
        private int FindLineSeparatorMultiSegment(ReadOnlySpan<byte> localBuffer, ref int dangerousLineSeparatorBytesConsumed)
        {
            ...
            while (true)
            {
                int idx = localBuffer.IndexOfAny(JsonUtf8Constant.LineFeed, JsonUtf8Constant.CarriageReturn, JsonSharedConstant.StartingByteOfNonStandardSeparator);
                ...
                if (localBuffer[idx] != JsonSharedConstant.StartingByteOfNonStandardSeparator)
                {
                    return totalIdx + idx;
                }
        ...
        // assumes first byte (JsonUtf8Constant.UnexpectedEndOfLineSeparator) is already read
        ...
        private bool SkipMultiLineCommentMultiSegment(ReadOnlySpan<byte> localBuffer)
        {
            ...
            while (true)
            {
                ...
                if (expectSlash)
                {
                    if (localBuffer[0] == JsonUtf8Constant.Slash)
                    {
                        _consumed++;
                        _bytePositionInLine++;
                        return true;
                ...
                int idx = localBuffer.IndexOfAny(JsonUtf8Constant.Asterisk, JsonUtf8Constant.LineFeed, JsonUtf8Constant.CarriageReturn);
                ...
                if (idx != -1)
                {
                    ...
                    switch (marker)
                    {
                        ...
                        case JsonUtf8Constant.LineFeed:
                            ...
                            break;
                        default:
                            Debug.Assert(marker == JsonUtf8Constant.CarriageReturn);
                            _bytePositionInLine = 0;
                            _lineNumber++;
