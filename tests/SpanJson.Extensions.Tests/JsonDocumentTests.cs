...
namespace SpanJson.Tests
{
    public static class JsonDocumentTests
    {
        ...
        {
            using (JsonDocument doc = JsonDocument.Parse(TestSR.SimpleArrayJson))
            {
                JsonElement root = doc.RootElement;
        ...
        {
            using (JsonDocument doc = JsonDocument.Parse(TestSR.SimpleObjectJson))
            {
        ...
        {
            using (JsonDocument doc = JsonDocument.Parse(TestSR.ParseJson))
            {
                JsonElement parsedObject = doc.RootElement;
        ...
        public static void JsonArrayToString()
        {
            using (JsonDocument doc = JsonDocument.Parse(TestSR.ParseJson))
            {
                ...
                Assert.Equal(TestSR.ParseJson, root.ToString());
            }
        }
        ...
        {
            using (JsonDocument doc = JsonDocument.Parse(TestSR.BasicJson))
            {
                JsonElement root = doc.RootElement;

                Assert.Equal(JsonValueKind.Object, root.ValueKind);
                Assert.Equal(TestSR.BasicJson, root.ToString());
            }
        }
        ...
        public static void CheckUseDefault()
        {
            ...
            try
            {
                Utf8JsonWriter writer = new Utf8JsonWriter(1024);
                root.WriteTo(ref writer);
                Assert.True(false);
            }
            catch (Exception ex)
            {
    ...
    ""glub"": { ""bool"": false }
        ...
        public static void ValueEquals_Null_TrueForNullFalseForEmpty()
        {
            using (JsonDocument doc = JsonDocument.Parse("   null   "))
            {
                ...
                Assert.False(jElement.ValueEquals(JsonHelpers.Empty<byte>()));
        ...
        public static void ValueEquals_EmptyJsonString_True()
        {
            using (JsonDocument doc = JsonDocument.Parse("\"\""))
            {
                ...
                Assert.True(jElement.ValueEquals(default(ReadOnlySpan<char>)));
                Assert.True(jElement.ValueEquals(default(ReadOnlySpan<byte>)));
                Assert.True(jElement.ValueEquals(JsonHelpers.Empty<byte>()));
            }
        }
        ...
        public static void WriteNumberTooLargeScientific()
        {
            ...
            {
                var writer = new Utf8JsonWriter(1024, default);
                doc.WriteTo(ref writer);

                AssertContents(expectedNonIndentedJson, writer.ToByteArray());
            }
        }
